...
const importMetaEnvRestriction = {
  ...
  // dev/prod boundary — docs/dev/tooling.md). All other `import.meta.env`
...
// gate and risks shipping dev code (docs/dev/tooling.md). Matched by regex
...
const devImportRestriction = {
  ...
  message: 'Production code must not import a dev/ module directly — reach dev tooling through its seam (docs/dev/tooling.md).',
};
