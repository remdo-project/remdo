...
import type { DateQuerySession } from './types';

interface ResolvedDateQuerySession {
  session: DateQuerySession;
...
function getPreviousTextCharacter(node: TextNode, offset: number): string | null {
  if (offset > 0) {
    return node.getTextContent()[offset - 1] ?? null;
  }
...
function hasTriggerBoundary(triggerNode: TextNode, triggerOffset: number): boolean {
  const previousCharacter = getPreviousTextCharacter(triggerNode, triggerOffset);
...
function inferSessionFromAnchor(anchorNode: TextNode, caretOffset: number): DateQuerySession | null {
  ...
  while (current) {
    ...
    for (let index = startOffset; index >= 0; index -= 1) {
      if (text[index] === '!' && hasTriggerBoundary(current, index)) {
...
export function $resolveDateQuerySession(
  anchorNode: TextNode,
  caretOffset: number,
  currentSession: DateQuerySession | null
): ResolvedDateQuerySession | null {
  ...
  if (!hasTriggerBoundary(triggerNode, session.triggerOffset)) {
    return null;
  }
