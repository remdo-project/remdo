...
describe('current user bootstrap cache', () => {
  ...
  it('stores a successful bootstrap response for later offline use', async () => {
    ...
    const { getCachedCurrentUserBootstrap, getCurrentUserBootstrap } = await import('#client/app/user-data/current-user-bootstrap');
  ...
  it('uses the cached bootstrap when offline with remembered auth', async () => {
    ...
    let bootstrapModule = await import('#client/app/user-data/current-user-bootstrap');
  ...
  it('clears the memory and durable bootstrap cache', async () => {
    ...
    const { clearCurrentUserBootstrapCache, getCachedCurrentUserBootstrap, getCurrentUserBootstrap } = await import('#client/app/user-data/current-user-bootstrap');
    await expect(getCurrentUserBootstrap()).resolves.toEqual(BOOTSTRAP);
