...
import { isExactHttpOrigin, normalizeToHttpOrigin } from '#platform/net/http-origin';

describe('isExactHttpOrigin', () => {
  it('accepts bare http and https origins', () => {
    expect(isExactHttpOrigin('https://source.example')).toBe(true);
    expect(isExactHttpOrigin('http://localhost:4000')).toBe(true);
    expect(isExactHttpOrigin('https://source.example:8443')).toBe(true);
  });

  it('rejects non-http(s) schemes even when URL.origin equals the input', () => {
    expect(isExactHttpOrigin('ws://source.example')).toBe(false);
    expect(isExactHttpOrigin('ftp://source.example')).toBe(false);
    expect(isExactHttpOrigin('javascript:alert(1)//')).toBe(false);
  });

  it('rejects anything that is more than a bare origin', () => {
    expect(isExactHttpOrigin('https://source.example/path')).toBe(false);
    expect(isExactHttpOrigin('https://source.example?x=1')).toBe(false);
    expect(isExactHttpOrigin('not-a-url')).toBe(false);
    expect(isExactHttpOrigin('')).toBe(false);
  });
});
