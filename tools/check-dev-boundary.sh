#!/usr/bin/env sh
# Guards the dev/prod boundary (docs/dev/tooling.md): dev-only tooling must
...
if [ -n "$leaked" ]; then
  ...
  echo "check-dev-boundary: a dev surface is reachable from production code — gate it behind import.meta.env.DEV (see docs/dev/tooling.md)." >&2
  exit 1
fi
