...
# (docs/architecture.md#production-bundle-boundary): dev-only tooling must
...
if [ -n "$leaked" ]; then
  ...
  echo "check-dev-boundary: a dev surface is reachable from production code — gate it behind import.meta.env.DEV (see docs/architecture.md#production-bundle-boundary)." >&2
  exit 1
fi
