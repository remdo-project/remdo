...
The read-only agent runner runs one provider-native command in a fresh agent
session rooted at the caller's current repository. It returns invocation
evidence without interpreting the command's subject or result. It does not
resolve a Git scope, decide whether the requested operation completed, or run a
free-form prompt.

## Request

The caller selects a supported agent adapter and supplies one native command,
its ordered arguments, and any command-specific instructions or final-response
schema. The adapter owns the executable and all isolation, permission, session,
and output-capture settings; caller input cannot replace or weaken them.
...
cleanly and yield one non-empty final agent response. Intermediate execution,
transport completion alone, and a process exit without a usable final response
...
response content for the caller to interpret.
