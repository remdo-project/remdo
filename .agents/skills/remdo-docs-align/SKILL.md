...
1. **Gates.** Run `pnpm run lint:md` (the product's style/link gate) and
   ...
   over `CONTRIBUTING.md` and `docs/` prose except specification feedback cases;
   skill-file prose is covered by the align/advocate stages, not the gates) and
   fix to clean. On a scope narrower than the corpus,
