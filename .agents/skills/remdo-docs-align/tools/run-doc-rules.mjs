...
// rules (temporal wording, References shape) over consolidated docs/ prose.
// Pending OpenSpec material retains historical and pre-consolidation wording,
// so only the product style/link gate covers it. Deliberately bypasses
// markdownlint-cli2 config discovery (the repo config is the product gate and
// would shadow this one); style/link linting stays `pnpm run lint:md`.
...
const pendingOpenSpec = 'docs/pending-review/openspec/';
const files = fs
  .globSync('docs/**/*.md', { cwd: process.cwd() })
  .filter((file) => !file.startsWith(pendingOpenSpec));
if (files.length === 0) {
