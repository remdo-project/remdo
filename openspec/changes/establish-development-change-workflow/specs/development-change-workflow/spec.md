...
When a completed OpenSpec change is ready to archive and its delta specs are
already synchronized or intentionally have no durable-spec impact, the agent
MUST treat `--skip-specs` archival and its archive-only local commit as one
action. This rule MUST NOT imply that other archives skip spec updates.

#### Scenario: Agent recommends archive-only finalization

- **WHEN** the agent determines that `--skip-specs` is appropriate and archival
...
- **THEN** the agent confirms the delta condition, archives the change, runs
  strict OpenSpec validation, stages only the archive move, and commits it
