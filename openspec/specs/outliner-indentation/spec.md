...
outdent it with `Shift+Tab`. A caret or inline text selection in note content or
a body targets its owning note; a structural selection targets its selected
note range.

#### Scenario: Indent from a non-structural selection

- **WHEN** a user presses `Tab` from a caret or inline text range in a note's
  content or body
