...
generic URL links. [Links](./links.md) owns classification between the link kinds
and generic URL-link behavior.
...
1. RemDo-owned links target stable note identity (`docId` + `noteId`), not the
   visible link text.
2. Note links are created inline through `@`, an inline trigger character; its
   open/close/confirm lifecycle is the shared one in
   [Editor popups](./popups.md). This specification defines only what differs.
3. The query is the text after `@` in the [pinned span](./popups.md#shared-editor-popup-contract), length minimum 0, so
   results may appear immediately. Whitespace is allowed in the query.
4. A note-link's insertion path chooses its display text, which is then stored
   locally; later target renames do not update it.
5. Note-link clicks use native `href` navigation semantics and route handling.
6. Creating a note link from a RemDo-owned plain-text note URL, by paste or
   [link controls](./links.md#generic-url-authoring), inserts a note-link node.
   Its initial label is selected text when it contains a non-whitespace
   character, the target note title when available, or otherwise the entered URL
   string. Paste commits that label directly; link controls expose it for editing
   and retain the edited value.
7. Typing a URL — including a same-origin RemDo note URL — does not create
   note-link identity; the note-link upgrade applies only to paste and link
   controls.
8. Clipboard payloads (copy/cut) must include explicit `docId` for every note
   link so cross-context paste has complete target identity.
9. Cross-document pastes preserve source-target link identity; note links are
   not retargeted to the destination document.
