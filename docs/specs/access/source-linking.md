...
- Resolve direct headless development origins such as `http://athena` before
  rejecting non-loopback HTTP sources during source derivation and removing
  source-issuer normalization. Docker E2E already uses loopback.
