...
document grants, and issues
[Y-Sweet document client tokens](../../architecture.md#token-vocabulary).

A **home server** stores the user's OAuth account tokens for linked sources and
presents source documents alongside its local documents. Home and source
accounts are independent identities; their email addresses do not need to
match.
...
the browser subscribes to the source-owned user-data projection and merges its
...
callback URI. Registration becomes usable without restarting the home. Later
links to the same origin reuse the cached client across home users.
...
one. A signed-in user can distinguish a known but unlinked source from an
unknown source when they already know its origin; this reveals no user or
document data.
