...
`uncommitted` contains changes in the current tracked files, plus untracked
files not excluded by Git's standard ignore rules. A repository is dirty when
standard Git status reports staged, unstaged, or untracked work.
